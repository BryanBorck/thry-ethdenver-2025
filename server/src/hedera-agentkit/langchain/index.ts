...
`;
  ...
  protected async _call(input: string): Promise<string> {
    try {
      ...
      const tokenId = (
        await this.hederaKit.createNFT({
          name: parsedInput.name,
          symbol: parsedInput.symbol,
  ...
  protected async _call(input: string): Promise<string> {
    try {
      ...
      return JSON.stringify({
        ...
        totalAmount: parsedInput.recipients.reduce(
...
`;
  ...
  protected async _call(input: string): Promise<string> {
    try {
      ...
      await this.hederaKit.rejectToken(TokenId.fromString(parsedInput.tokenId));
...
`;
  ...
  protected async _call(input: string): Promise<string> {
    try {
      const parsedInput = JSON.parse(input);
      await this.hederaKit.deleteTopic(TopicId.fromString(parsedInput.topicId));
      return JSON.stringify({
        status: "success",
